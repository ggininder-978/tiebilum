...
### 4. 品牌證據與傳承 (Evidence & Heritage)

位於銷售診斷下方，作為公開的品牌事實庫：

- **歷史事實 (History Facts)**：以時間軸或卡片呈現經考據的品牌大事記。
- **風土證據 (Terroir Evidence)**：呈現埔里風土與 Badila 紅甘蔗的科學或歷史優勢。
- **可用文案庫 (Usable Copy)**：提供經核准的短句，供官網上架或 CIS 設計使用。
- **禁忌與待查 (Guardrails)**：列出嚴禁使用的虛浮詞彙、未經證實的宣稱，以及目前尚在考據中的疑點。
- **來源追溯**：所有事實必須回連至對應的 Wiki 實體頁面。

### 5. QDM 頁面交付清單 (QDM Page Readiness)

位於品牌證據下方，專為官網與 CIS 團隊設計：

- **首頁文案包 (Homepage Bundle)**：包含主標、副標、簡介與 CTA 按鈕文字。
- **商品資訊卡 (Product Details)**：提供 Top 5 SKUs 的品名、規格、風味描述與注意事項。
- **視覺素材清單 (Asset Gaps)**：盤點各頁面所需的照片與影像狀態（可用/缺漏）。
- **品牌禁忌 (Guardrails)**：明確條列 QDM 平台嚴禁使用的行銷詞彙。
